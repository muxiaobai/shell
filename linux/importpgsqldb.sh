...
if [ $pgsqlstatus -gt 1 ]; then
	...
	su - pgsql -c "export PGPASSWORD='pgsql'; /opt/openlab/pgsql/bin/createdb  -h 127.0.0.1 -p 15432 -U pgsql apptest"
	su - pgsql -c "export PGPASSWORD='pgsql'; /opt/openlab/pgsql/bin/psql  -h 127.0.0.1 -p 15432 -d apptest -U pgsql -f ${temppath}/apptest.sql"
else
