...
ssh -p ${PORT} root@${IP} "cd ~; cd ${dist_path};chmod +x ${dist_path}/shutdown.sh;${dist_path}/shutdown.sh ${IP} ${dist_path} ;"

if [ $? -eq 0 ];then
    echo -e "\033[1;32;40m ${IP} ssh stop shell is success,path:${dist_path} \033[0m" 
else 
    echo -e "\033[1;31;40m ${IP} ssh stop shell is failed,path: ${dist_path}  \033[0m" 
    exit 1
fi
done
